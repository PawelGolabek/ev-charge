...
async function getWalletSetupFiles(walletSetupDirPath) {
  await fs6.access(walletSetupDirPath).catch((e) => {
  ...
  const fileList = (await fs6.readdir(walletSetupDirPath)).filter(setupFilesFilter);
  if (!fileList.length) {
    throw new Error(
      [
...
function ensureCacheDirExists() {
  ...
  fs6.ensureDirSync(cacheDirPath);
  return cacheDirPath;
}
...
async function downloadFile(options) {
  ...
  const returnPromise = new Promise((resolve6, reject) => {
    ...
    axios.get(url, {
      ...
      const writer = fs6.createWriteStream(filePath);
...
async function unzipArchive(options) {
  ...
  const resultPromise = new Promise((resolve6, reject) => {
    ...
    const fileExists = fs6.existsSync(outputPath);
    ...
    fs6.createReadStream(archivePath).pipe(unzippper.Parse()).on("entry", function(entry) {
      const fileName = entry.path;
      const type = entry.type;
      if (type === "Directory") {
        fs6.mkdirSync(path5.join(outputPath, fileName), { recursive: true });
        return;
      }
      if (type === "File") {
        const outputFilePath = path5.join(outputPath, fileName);
        const outputFilePathDir = path5.dirname(outputFilePath);
        if (!fs6.existsSync(outputFilePathDir)) {
          fs6.mkdirSync(outputFilePathDir, { recursive: true });
        }
        entry.pipe(fs6.createWriteStream(outputFilePath));
      }
    }).promise().then(() => {
      resolve6({
        outputPath,
...
var createGlobPattern = (walletSetupDir) => path5.join(walletSetupDir, "**", "*.{js,ts}");
...
`);
  const program = new Command().name(chalk.magenta("core")).description("A CLI for building the cache of wallet setup functions").argument("[dir]", "Directory containing the wallet setup functions", path5.join("test", WALLET_SETUP_DIR_NAME)).option(
    "--headless",
